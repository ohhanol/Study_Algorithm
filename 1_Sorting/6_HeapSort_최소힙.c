...
처리: n개 랜덤 수 발생, n개 랜덤 수를 힙정렬 알고리즘으로 오름 정렬
...
void heapify(int arr[], int k, int n) {				//k는 내가 쳐다볼 노드, n은 최대 인덱스(arr배열의 전체 크기)
	...
	else if (left <= n) smaller = left;				//k가 왼쪽 자식만 있는 경우
	else return;									//k가 자식이 없는 경우


	if (arr[smaller] < arr[k]) {						//찾은 smaller가 k보다 작으면
		tmp = arr[k];
		arr[k] = arr[smaller];
		arr[smaller] = tmp;
		heapify(arr, smaller, n);					//자리가 바뀐 smaller을 기준으로 다시 수선작업(heapify)
	}
}
void print(int arr[], int n) {
	for (int i = 0; i < n; i++) {
